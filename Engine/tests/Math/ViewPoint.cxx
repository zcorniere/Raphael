...
TEST_CASE("Viewpoint Tests - float")
{
...
TEST_CASE("Viewpoint Tests - double")
{
    const double Epsilon = 1e-12f;

    const DVector3 Location(GENERATE(take(1, random(-50.0f, 50.0f))), GENERATE(take(1, random(-50.0f, 50.0f))),
                            GENERATE(take(1, random(-50.0f, 50.0f))));
    const double fAspectRatio = 16.0f / 9.0f;
    const double fFOV = GENERATE(take(1, random(0.0f, 180.0f)));
    const double fFar = 100.0f;
    const double fNear = 0.1f;

    Math::TViewPoint<double> Viewpoint(fFOV, fNear, fFar, fAspectRatio);
    // Viewpoint.SetLocation(Location);

    SECTION("Projection Matrix")
    {
        DMatrix4 ProjectionMatrix = Viewpoint.GetProjectionMatrix();

        glm::dmat4 ExpectedProjectionMatrix = glm::perspective(glm::radians(fFOV), fAspectRatio, fNear, fFar);
        ExpectedProjectionMatrix[1][1] *= -1;
        ...
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                INFO("Result[" << i << "][" << j << "]: " << ProjectionMatrix[i][j]);
                INFO("ExpectedResult[" << i << "][" << j << "]: " << ExpectedProjectionMatrix[i][j]);
                CHECK_THAT(ProjectionMatrix[i][j], Catch::Matchers::WithinRel(ExpectedProjectionMatrix[i][j], Epsilon));
            }
        }
    }
    ...
    // SECTION("View Matrix")
    // {
    //     DMatrix4 ViewMatrix = Viewpoint.GetViewMatrix();

    //     glm::dvec3 ExpectedLocation(Location.x, Location.y, Location.z);
    //     glm::dvec3 Center = {ExpectedLocation.x + Math::FrontVector.x, ExpectedLocation.y + Math::FrontVector.y,
    //                         ExpectedLocation.z + Math::FrontVector.z};
    //     glm::dvec3 Up = {Math::UpVector.x, Math::UpVector.y, Math::UpVector.z};
    //     glm::dmat4 ExpectedViewMatrix = glm::lookAt(ExpectedLocation, Center, Up);
    ...
    //             INFO("Result[" << i << "][" << j << "]: " << ViewMatrix[i][j]);
    //             INFO("ExpectedResult[" << i << "][" << j << "]: " << ExpectedViewMatrix[i][j]);
    //             CHECK_THAT(ViewMatrix[i][j], Catch::Matchers::WithinRel(ExpectedViewMatrix[i][j], Epsilon));
    //         }
    //     }
    // }
    ...
    // SECTION("ViewProjmatrix")
    // {
    //     DMatrix4 ViewProjMatrix = Viewpoint.GetViewProjectionMatrix();

    //     glm::dvec3 ExpectedLocation(Location.x, Location.y, Location.z);
    //     glm::dvec3 Center = {ExpectedLocation.x + Math::FrontVector.x, ExpectedLocation.y + Math::FrontVector.y,
    //                         ExpectedLocation.z + Math::FrontVector.z};
    //     glm::dvec3 Up = {Math::UpVector.x, Math::UpVector.y, Math::UpVector.z};
    //     glm::dmat4 ExpectedViewProjMatrix =
    //         glm::perspective(glm::radians(fFOV), fAspectRatio, fNear, fFar) * glm::lookAt(ExpectedLocation, Center,
    //         Up);
    ...
    //             INFO("Result[" << i << "][" << j << "]: " << ViewProjMatrix[i][j]);
    //             INFO("ExpectedResult[" << i << "][" << j << "]: " << ExpectedViewProjMatrix[i][j]);
    //             CHECK_THAT(ViewProjMatrix[i][j], Catch::Matchers::WithinRel(ExpectedViewProjMatrix[i][j], Epsilon));
    //         }
    //     }
    // }
}
