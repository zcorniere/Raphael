...
DECLARE_LOGGER_CATEGORY(Core, LogVulkan, Warn);
...
constexpr std::string_view VkObjectTypeToString(const VkObjectType type)
{
    switch (type) {
...
inline void VulkanCheckResult(VkResult result, const std::source_location &location = std::source_location::current())
{
    if (result != VK_SUCCESS) {
        LOG(LogVulkan, Error, "VkResult is '{0}' in {1}:{2}", VKResultToString(result), location.file_name(),
            location.line());
